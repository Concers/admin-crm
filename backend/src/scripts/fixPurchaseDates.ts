/**
 * One-off fix: Excel / eski form kayıtlarında alım tarihleri 1 gün geride kalmış.
 * Tüm purchase.date (ve varsa dueDate) değerlerine +1 takvim günü ekler.
 *
 * Kullanım:
 *   npx tsx src/scripts/fixPurchaseDates.ts          # önizleme
 *   npx tsx src/scripts/fixPurchaseDates.ts --apply    # uygula
 */
...
async function main() {
  ...
  console.log(`\n${changed} alım kaydı ${apply ? "güncellendi" : "güncellenecek"}.`);
  if (!apply) {
    console.log("Onaylamak için: npm run db:fix-purchase-dates -- --apply");
  }
}
