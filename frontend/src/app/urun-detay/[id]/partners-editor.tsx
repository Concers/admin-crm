...
import type { Partner, ProductPartnerLink } from "@/lib/api";
import { PARTNER_ROL_ETIKET } from "@/lib/urun-detay-fields";
import { addProductPartnerAction, deleteProductPartnerAction } from "../actions";

const ROLE_TONE: Record<string, "green" | "amber" | "blue"> = {
  SUPPLIER: "green",
  POTENTIAL_SUPPLIER: "amber",
  CUSTOMER: "blue",
};
    ...
    <div className="space-y-4">
            ...
            <li key={pl.id} className="flex items-center justify-between gap-3 px-3 py-2 text-sm">
              <div className="flex items-center gap-2 min-w-0">
                <Badge tone={ROLE_TONE[pl.role] ?? "default"}>
                  {PARTNER_ROL_ETIKET[pl.role] ?? pl.role}
                </Badge>
              ...
              <Button
                ...
                disabled={pending}
                onClick={() =>
                  run(() => deleteProductPartnerAction(productId, pl.id), { success: "Cari bağı silindi." })
                }
                aria-label="Sil"
              >
      ...
      <form
        onSubmit={(e) => {
          ...
          run(() => addProductPartnerAction(productId, new FormData(form)), {
      ...
      >
        ...
        <Select name="role" defaultValue="SUPPLIER" aria-label="Rol">
          ...
          <option value="POTENTIAL_SUPPLIER">Potansiyel Tedarikçi</option>
