...
const CELL_RENDERERS: Record<string, (row: FaturaTableRow) => ReactNode> = {
  faturaNo: (r) =>
    r.faturaNo !== "—" ? (
  ...
  kdvDahil: (r) => (
    <span className="font-semibold tabular-nums text-blue-700">{r.kdvDahil}</span>
  ),
  notlar: (r) =>
    r.notlar ? (
      <span
...
export function FaturaWorkspace({
  rows,
  partners,
  products,
}: {
  ...
  const [editing, setEditing] = useState<FaturaTableRow | null>(null);

  return (
    <>
      <RecordWorkspaceToolbar
        addLabel="Yeni Fatura Ekle"
        hint="Satıra tıklayarak düzenleyebilir veya yeni fatura ekleyebilirsiniz."
        onAdd={() => setCreateOpen(true)}
      />
      <FaturaList rows={rows} onEdit={setEditing} />
      {createOpen && (
        <FaturaModal
          mode="create"
          partners={partners}
...
function FaturaList({
  ...
  rows: FaturaTableRow[];
  onEdit: (row: FaturaTableRow) => void;
}) {
  const columns = useMemo(
    () => [
      ...COLUMNS.map((col) => ({
        ...
        sortValue:
          col.key === "tarih"
            ? (r: FaturaTableRow) => r._date
            : col.key === "vade"
              ? (r: FaturaTableRow) => r._dueDate ?? ""
              : col.key === "toplam"
                ? (r: FaturaTableRow) => r._totalAmount
                : col.key === "kdvDahil"
                  ? (r: FaturaTableRow) => r._vatIncludedAmount
                  : col.key === "kalemSayisi"
                    ? (r: FaturaTableRow) => r.kalemSayisi
                    : undefined,
        align:
      ...
      })),
      {
        ...
        render: (row: FaturaTableRow) => <RowActions row={row} onEdit={() => onEdit(row)} />,
      },
    ],
    [onEdit]
  );
  ...
  return (
    <DataTable
      ...
      columns={columns}
      emptyText="Henüz fatura kaydı yok"
      emptyHint="Yukarıdaki butonla ilk faturanızı ekleyebilirsiniz."
    />
  );
}

function RowActions({ row, onEdit }: { row: FaturaTableRow; onEdit: () => void }) {
  ...
  return (
    <div className="flex items-center gap-0.5" onClick={(e) => e.stopPropagation()}>
      ...
      <Button
        ...
        onClick={() => {
          const label = row.faturaNo !== "—" ? row.faturaNo : row.cari;
          if (confirm(`"${label}" faturası silinsin mi?`)) {
            run(() => deleteFatura(row.id), { success: "Fatura silindi." });
          }
        }}
      >
